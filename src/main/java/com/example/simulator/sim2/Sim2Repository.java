...
public interface Sim2Repository extends org.springframework.data.repository.Repository<SimulationRun, UUID> {
	...
	@Query(value = """
			...
			WHERE sr.simulation_id = :simulationId
			  AND cs.round_number = 0
			  AND cs.status = 'SCORED'
			  -- A terminated run drops off the console and out of the finished-teams list; exclude it
			  -- from the debrief dashboard and leaderboard too, so a team terminated after it finalised
			  -- (for example to let it re-run) does not linger in the cohort standings.
			  AND sr.status <> 'TERMINATED'
