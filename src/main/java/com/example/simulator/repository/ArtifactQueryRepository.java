...
public interface ArtifactQueryRepository extends org.springframework.data.repository.Repository<SimulationRun, UUID> {
	...
	// =========================
	@Query(value = """
			SELECT ria.injection_id AS "injectionId",
			       ria.round_number AS "roundNumber",
			       ria.title        AS "title",
			       ria.content      AS "content",
			       ria.injected_at  AS "injectedAt"
			FROM run_injected_artifacts ria
			JOIN simulation_runs sr ON sr.run_id = ria.run_id
			-- Terminating a run ends it for good: injected notes must stop firing too, not just the
			-- authored feed (which is already guarded in the visibility query).
			WHERE ria.run_id = :runId
			  AND sr.status <> 'TERMINATED'
			ORDER BY ria.injected_at
			""", nativeQuery = true)
